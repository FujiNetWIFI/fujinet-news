#!/bin/sh
# run.sh -- build (if needed) and launch FujiNet News in the FujiNet-patched
...
# last built.
if [ ! -f news.rom ] || [ -n "$(find . -maxdepth 1 -name '*.bas' -newer news.rom)" ]; then
    echo "Building news.rom..."
    make
fi
